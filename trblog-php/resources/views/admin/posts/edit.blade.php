    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.posts.update', $post) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="title" name="title" class="w-full px-4 py-2 border border-gray-300 rounded" value="{{ old('title', $post->title) }}">
                @error('title')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="slug" name="slug" class="w-full px-4 py-2 border border-gray-300 rounded" value="{{ old('slug', $post->slug) }}">
                @error('slug')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="excerpt" name="excerpt" class="w-full px-4 py-2 border border-gray-300 rounded" rows="3">{{ old('excerpt', $post->excerpt) }}</textarea>
            </div>
            
            <div class="mb-4">
                <label for="content" class="block text-gray-700 font-medium mb-2">内容</label>
                <textarea id="content" name="content" class="w-full px-4 py-2 border border-gray-300 rounded" rows="10">{{ old('content', $post->content) }}</textarea>
                @error('content')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="status" name="status" class="w-full px-4 py-2 border border-gray-300 rounded">
                    <option value="published" {{ (old('status', $post->status) === 'published') ? 'selected' : '' }}>已发布</option>
