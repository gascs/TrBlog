...
import { Folder, Calendar, User, Eye, MessageSquare, ArrowLeft } from 'lucide-react';
...
const CategoryDetailPage: React.FC = () => {
  ...
  const { data: postsData, isLoading: postsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      const response = await api.get('/posts', { params: { categoryId: id } });
      return response.data;
    },
    enabled: !!id,
  });
      ...
      <div className="mb-8">
        ...
        {posts.length > 0 ? (
          <div className="space-y-4">
            {posts.map((post: Post, index: number) => (
              ...
              >
                <Link
                  to={`/posts/${post.id}`}
                  className="block bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-700 hover:shadow-lg transition-all duration-300"
                >
          ...
          >
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">该分类下暂无文章</h3>
            <p className="text-gray-600 dark:text-gray-400">还没有任何文章发布在这个分类下</p>
          </motion.div>
        )}
      </div>
    </motion.div>
